...
//混控任务
const uint16_t THROTTLE_OutputCode[513] =      //油门摇杆ADC映射表
{
...
void mixesTask(void* param)
{
    ...
    if(JoystickDeadZonePercent > MaxDeadZonePercent)
    {
    ...
    while(1)
    {
        ...
        for(mixIndex = 0;mixIndex < 8;mixIndex++)
        {
            ...
            if(mixData[mixIndex].gimbalChannel < 4)/*选取的为摇杆通道值，设置混控*/
            {
                ...
                {
                    /*油门通道映射*/
                    mixData[mixIndex].output = Mixes_Gimbalreverse(mixData[mixIndex].reverse, mixData[mixIndex].output, (uint16_t*)THROTTLE_OutputCode);
                }
                else
                {
                    /*非油门通道映射*/
